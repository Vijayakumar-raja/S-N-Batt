...
#define ALERT_STATE_HIGH1                     3

#define VIBRATION_MSG_COUNT 1
#define VIBRATION_NOTF_COUNT 3
...
// GAP GATT Attributes
static uint8 attDeviceName[GAP_DEVICE_NAME_LEN] = "Sync";
...
uint16 KeyFobApp_ProcessEvent( uint8 task_id, uint16 events )
{
  ...
  if ( events & KFD_SOS_EVENT_SEND )
  {
    ...
    return (events ^ KFD_SOS_EVENT_SEND);
  }
  ...
  if ( events & KFD_TOGGLE_BUZZER_EVT )
  {
    ...
    if ( buzzer_state == BUZZER_ON )
    {
      ...
      if(( ( keyfobProxIMAlertLevel==PP_ALERT_LEVEL_LOW ) && (buzzer_beep_count==1) )|| (( keyfobProxIMAlertLevel==PP_ALERT_LEVEL_HIGH ) && (buzzer_beep_count==3)))
{
      keyfobapp_StopAlert();
      ...
      // if it has, then don't turn it back on
         if(keyfobProxIMAlertLevel==PP_ALERT_LEVEL_LOW)
         {
      
        if ( ( buzzer_beep_count < VIBRATION_MSG_COUNT ) &&
           ( ( keyfobProximityState == KEYFOB_PROXSTATE_LINK_LOSS ) ||
             ( keyfobProximityState == KEYFOB_PROXSTATE_PATH_LOSS )    ) )
      {
        osal_start_timerEx( keyfobapp_TaskID, KFD_TOGGLE_BUZZER_EVT, 300 );
      }
         }
         
         if(keyfobProxIMAlertLevel==PP_ALERT_LEVEL_HIGH1)
         {
      
        if ( ( buzzer_beep_count < VIBRATION_NOTF_COUNT ) &&
           ( ( keyfobProximityState == KEYFOB_PROXSTATE_LINK_LOSS ) ||
             ( keyfobProximityState == KEYFOB_PROXSTATE_PATH_LOSS )    ) )
      {
        osal_start_timerEx( keyfobapp_TaskID, KFD_TOGGLE_BUZZER_EVT, 300 );
...
static void keyfobapp_HandleKeys( uint8 shift, uint8 keys )
{
  ...
  if ( keys & HAL_KEY_SW_1 )
  {
    ...
    // State=^State;
      ...
      osal_start_timerEx( keyfobapp_TaskID, KFD_SOS_EVENT_SEND, 5000 );
...
static void keyfobapp_PerformAlert( void )
{
  ...
  else if ( keyfobProximityState == KEYFOB_PROXSTATE_PATH_LOSS )
  {
    ...
    {
    case PP_ALERT_LEVEL_LOW:
// Messages - 1 vibration blue flash
      #if defined ( POWER_SAVING )
        osal_pwrmgr_device( PWRMGR_ALWAYS_ON );
      #endif
      ...
      buzzer_state = BUZZER_ON;
      // only run buzzer for 200ms
      osal_start_timerEx( keyfobapp_TaskID, KFD_TOGGLE_BUZZER_EVT, 1000 );

      HalLedSet( HAL_LED_2, HAL_LED_MODE_ON );
      break;


    case PP_ALERT_LEVEL_HIGH1:
// Notifications 3 Vibration Green flash
      #if defined ( POWER_SAVING )
        osal_pwrmgr_device( PWRMGR_ALWAYS_ON );
      #endif

      keyfobAlertState = ALERT_STATE_HIGH1;

      buzzerStart( BUZZER_ALERT_HIGH_FREQ );
      buzzer_state = BUZZER_ON;
      // only run buzzer for 200ms
      osal_start_timerEx( keyfobapp_TaskID, KFD_TOGGLE_BUZZER_EVT, 500 );
